<?php
/**
 * View: Technologies management screen.
 *
...
$edit_term = $edit_id ? get_term( $edit_id, 'epsw_technology' ) : null;
...
<?php include EPSW_PLUGIN_DIR . 'admin/views/partials/shell-header.php'; ?>
	<div class="epsw-admin-header">
		<h1><?php esc_html_e( 'Technologies', 'portfolio-showcase' ); ?></h1>
	</div>

	<div class="epsw-form-grid epsw-form-grid-narrow">
		<div class="epsw-admin-card">
			<form method="get">
				<input type="hidden" name="page" value="epsw-technologies" />
				<p class="search-box">
					<input type="search" name="s" value="<?php echo esc_attr( $search ); ?>" class="epsw-input" placeholder="<?php esc_attr_e( 'Search technologies…', 'portfolio-showcase' ); ?>" />
					<button type="submit" class="epsw-btn-admin epsw-btn-admin-ghost"><?php esc_html_e( 'Search', 'portfolio-showcase' ); ?></button>
				</p>
			</form>

			<table class="epsw-admin-table widefat" data-taxonomy="epsw_technology">
				<thead>
					<tr>
				...
				<tbody class="<?php echo empty( $search ) ? 'epsw-sortable-rows' : ''; ?>" id="epsw-sortable-technologies">
					<?php if ( ! is_wp_error( $terms ) && ! empty( $terms ) ) : ?>
						<?php foreach ( $terms as $term ) : ?>
							...
							$delete_url = wp_nonce_url(
								add_query_arg(
									array(
										'action' => 'epsw_delete_technology',
										'id'     => $term->term_id,
									),
									admin_url( 'admin-post.php' )
								),
								'epsw_delete_technology_' . $term->term_id
							);
							?>
							<tr data-term-id="<?php echo esc_attr( $term->term_id ); ?>">
								...
								<td>
									...
									|
									<a href="<?php echo esc_url( $delete_url ); ?>" class="epsw-row-delete" onclick="return confirm('<?php echo esc_js( __( 'Delete this technology?', 'portfolio-showcase' ) ); ?>');"><?php esc_html_e( 'Delete', 'portfolio-showcase' ); ?></a>
								</td>
							</tr>
						<?php endforeach; ?>
					<?php else : ?>
						<tr><td colspan="6"><?php esc_html_e( 'No technologies found.', 'portfolio-showcase' ); ?></td></tr>
					<?php endif; ?>
				</tbody>
			</table>
		...
		<div class="epsw-admin-card">
			<h2><?php echo $edit_term && ! is_wp_error( $edit_term ) ? esc_html__( 'Edit Technology', 'portfolio-showcase' ) : esc_html__( 'Add New Technology', 'portfolio-showcase' ); ?></h2>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				<input type="hidden" name="action" value="epsw_save_technology" />
				<input type="hidden" name="term_id" value="<?php echo esc_attr( $edit_term && ! is_wp_error( $edit_term ) ? $edit_term->term_id : 0 ); ?>" />
				<?php wp_nonce_field( 'epsw_save_technology' ); ?>

				<div class="epsw-field">
					<label for="epsw-tech-name"><?php esc_html_e( 'Technology Name', 'portfolio-showcase' ); ?></label>
					<input type="text" id="epsw-tech-name" name="name" required class="epsw-input" value="<?php echo esc_attr( $edit_term && ! is_wp_error( $edit_term ) ? $edit_term->name : '' ); ?>" />
				</div>

				<div class="epsw-field">
					<label><?php esc_html_e( 'Technology Icon', 'portfolio-showcase' ); ?></label>
					...
					<div class="epsw-media-uploader">
						<input type="hidden" name="icon_id" id="epsw-tech-icon-id" value="<?php echo esc_attr( $edit_icon_id ); ?>" />
						<div class="epsw-icon-preview" id="epsw-tech-icon-preview">
						...
						<button type="button" class="epsw-btn-admin epsw-btn-admin-ghost epsw-media-upload-btn" data-target-id="epsw-tech-icon-id" data-preview-id="epsw-tech-icon-preview">
							<?php echo $edit_icon_url ? esc_html__( 'Change Icon', 'portfolio-showcase' ) : esc_html__( 'Upload Icon', 'portfolio-showcase' ); ?>
						</button>
						<?php if ( $edit_icon_url ) : ?>
							<button type="button" class="epsw-btn-admin epsw-btn-admin-ghost epsw-media-remove-btn" data-target-id="epsw-tech-icon-id" data-preview-id="epsw-tech-icon-preview" style="margin-left: 10px;">
				...
				<button type="submit" class="epsw-btn-admin epsw-btn-admin-primary epsw-btn-full">
					<?php echo $edit_term && ! is_wp_error( $edit_term ) ? esc_html__( 'Update Technology', 'portfolio-showcase' ) : esc_html__( 'Add Technology', 'portfolio-showcase' ); ?>
				</button>
			</form>
		</div>
	</div>
